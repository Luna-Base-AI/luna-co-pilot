...
import { LunaIgnoreController } from "../../core/ignore/LunaIgnoreController"
...
export async function parseSourceCodeDefinitionsForFile(
	...
	if (ext === ".md" || ext === ".markdown") {
		...
		if (lunaIgnoreController && !lunaIgnoreController.validateAccess(filePath)) {
			return undefined
		}
	...
	const definitions = await parseFile(filePath, languageParsers, lunaIgnoreController)
	if (definitions) {
		return `# ${path.basename(filePath)}\n${definitions}`
	}
...
export async function parseSourceCodeForDefinitionsTopLevel(
	...
	const allowedFilesToParse = lunaIgnoreController ? lunaIgnoreController.filterPaths(filesToParse) : filesToParse
	...
	for (const file of markdownFiles) {
		// Check if we have permission to access this file
		if (lunaIgnoreController && !lunaIgnoreController.validateAccess(file)) {
			continue
		}
	...
	for (const file of otherFiles) {
		const definitions = await parseFile(file, languageParsers, lunaIgnoreController)
		if (definitions) {
			result += `# ${path.relative(dirPath, file).toPosix()}\n${definitions}\n`
		}
	}
 ...
 * @param lunaIgnoreController - Optional controller to check file access permissions
