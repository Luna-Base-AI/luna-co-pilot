...
type LunaCoPilotSettingsKey = keyof LunaCoPilotSettings
...
export class ContextProxy {
	...
	public setValue<K extends LunaCoPilotSettingsKey>(key: K, value: LunaCoPilotSettings[K]) {
		return isSecretStateKey(key) ? this.storeSecret(key, value as string) : this.updateGlobalState(key, value)
	}

	public getValue<K extends LunaCoPilotSettingsKey>(key: K): LunaCoPilotSettings[K] {
		return isSecretStateKey(key)
			? (this.getSecret(key) as LunaCoPilotSettings[K])
			: (this.getGlobalState(key) as LunaCoPilotSettings[K])
	}

	public getValues(): LunaCoPilotSettings {
		return { ...this.getAllGlobalState(), ...this.getAllSecretState() }
	}

	public async setValues(values: LunaCoPilotSettings) {
		const entries = Object.entries(values) as [LunaCoPilotSettingsKey, unknown][]
