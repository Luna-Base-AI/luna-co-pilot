...
const LUNA_MODES_FILENAME = ".lunamodes"
...
export class CustomModesManager {
	...
	private async getWorkspaceLunamodes(): Promise<string | undefined> {
		const workspaceFolders = vscode.workspace.workspaceFolders
		if (!workspaceFolders || workspaceFolders.length === 0) {
			return undefined
		}
		const workspaceRoot = getWorkspacePath()
		const lunamodesPath = path.join(workspaceRoot, LUNA_MODES_FILENAME)
		const exists = await fileExistsAtPath(lunamodesPath)
		return exists ? lunamodesPath : undefined
	}
	...
	private async loadModesFromFile(filePath: string): Promise<ModeConfig[]> {
		try {
			...
			const isLunamodes = filePath.endsWith(LUNA_MODES_FILENAME)
			const source = isLunamodes ? ("project" as const) : ("global" as const)
	...
	private async watchCustomModesFiles(): Promise<void> {
		...
		this.disposables.push(
			vscode.workspace.onDidSaveTextDocument(async (document) => {
				if (arePathsEqual(document.uri.fsPath, settingsPath)) {
					...
					// Get modes from .lunamodes if it exists (takes precedence)
					const lunamodesPath = await this.getWorkspaceLunamodes()
					const lunamodesModes = lunamodesPath ? await this.loadModesFromFile(lunamodesPath) : []

					// Merge modes from both sources (.lunamodes takes precedence)
					const mergedModes = await this.mergeCustomModes(lunamodesModes, result.data.customModes)
		...
		// Watch .lunamodes file if it exists
		const lunamodesPath = await this.getWorkspaceLunamodes()
		if (lunamodesPath) {
			this.disposables.push(
				vscode.workspace.onDidSaveTextDocument(async (document) => {
					if (arePathsEqual(document.uri.fsPath, lunamodesPath)) {
						const settingsModes = await this.loadModesFromFile(settingsPath)
						const lunamodesModes = await this.loadModesFromFile(lunamodesPath)
						// .lunamodes takes precedence
						const mergedModes = await this.mergeCustomModes(lunamodesModes, settingsModes)
						await this.context.globalState.update("customModes", mergedModes)
						await this.onUpdate()
					}
				}),
	...
	async getCustomModes(): Promise<ModeConfig[]> {
		...
		// Get modes from .lunamodes if it exists
		const lunamodesPath = await this.getWorkspaceLunamodes()
		const lunamodesModes = lunamodesPath ? await this.loadModesFromFile(lunamodesPath) : []
		...
		for (const mode of lunamodesModes) {
		...
		const mergedModes = [
			...lunamodesModes.map((mode) => ({ ...mode, source: "project" as const })),
			...settingsModes
	...
	async updateCustomMode(slug: string, config: ModeConfig): Promise<void> {
		try {
			...
			if (isProjectMode) {
				...
				const workspaceRoot = getWorkspacePath()
				targetPath = path.join(workspaceRoot, LUNA_MODES_FILENAME)
				const exists = await fileExistsAtPath(targetPath)
				logger.info(`${exists ? "Updating" : "Creating"} project mode in ${LUNA_MODES_FILENAME}`, {
					slug,
	...
	async deleteCustomMode(slug: string): Promise<void> {
		try {
			...
			const projectMode = lunamodesModes.find((m) => m.slug === slug)
			...
			await this.queueWrite(async () => {
				...
				if (projectMode && lunamodesPath) {
					await this.updateModesInFile(lunamodesPath, (modes) => modes.filter((m) => m.slug !== slug))
				}
