...
import { GlobalState, ProviderSettings, LunaCoPilotSettings } from "../../schemas"
import { t } from "../../i18n"
...
export class ClineProvider extends EventEmitter<ClineProviderEvents> implements vscode.WebviewViewProvider {
	public static readonly sideBarId = "luna-co-pilot.SidebarProvider" // used in package.json as the view's id. This value cannot be changed due to how vscode caches views based on their id, and updating the id would break existing instances of the extension.
	public static readonly tabPanelId = "luna-co-pilot.TabPanelProvider"
	...
	*/
	...
	async ensureMcpServersDirectoryExists(): Promise<string> {
		...
		if (process.platform === "win32") {
			// Windows: %APPDATA%\luna-co-pilot\MCP
			mcpServersDir = path.join(os.homedir(), "AppData", "Roaming", "luna-co-pilot", "MCP")
		} else if (process.platform === "darwin") {
			// macOS: ~/Documents/luna-co-pilot/MCP
			mcpServersDir = path.join(os.homedir(), "Documents", "luna-co-pilot", "MCP")
		} else {
			// Linux: ~/.local/share/luna-co-pilot/MCP
			mcpServersDir = path.join(os.homedir(), ".local", "share", "luna-co-pilot", "MCP")
		}
		...
		try {
			...
			return path.join(os.homedir(), ".luna-copilot", "mcp")
		}
	...
	async getStateToPostToWebview() {
		...
		const allowedCommands =
			vscode.workspace.getConfiguration("luna-co-pilot").get<string[]>("allowedCommands") || []
		const cwd = this.cwd
		...
		return {
			...
			telemetryKey,
			machineId,
			showLunaIgnoredFiles: showLunaIgnoredFiles ?? true,
			language,
	...
	async getState() {
		...
		return {
			...
			showLunaIgnoredFiles: stateValues.showLunaIgnoredFiles ?? true,
	...
	public async setValue<K extends keyof LunaCoPilotSettings>(key: K, value: LunaCoPilotSettings[K]) {
		await this.contextProxy.setValue(key, value)
	}

	public getValue<K extends keyof LunaCoPilotSettings>(key: K) {
		return this.contextProxy.getValue(key)
	}
	...
	public async setValues(values: LunaCoPilotSettings) {
