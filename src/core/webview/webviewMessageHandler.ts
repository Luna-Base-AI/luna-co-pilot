...
export const webviewMessageHandler = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	switch (message.type) {
		...
		case "openProjectMcpSettings": {
			...
			const lunaDir = path.join(workspaceFolder.uri.fsPath, ".luna")
			const mcpPath = path.join(lunaDir, "mcp.json")
		...
		case "language":
			...
			await provider.postStateToWebview()
			break
		case "showLunaIgnoredFiles":
			await updateGlobalState("showLunaIgnoredFiles", message.bool ?? true)
			await provider.postStateToWebview()
			break
		...
		case "humanRelayResponse":
			if (message.requestId && message.text) {
				vscode.commands.executeCommand("luna-co-pilot.handleHumanRelayResponse", {
					requestId: message.requestId,
					text: message.text,
					cancelled: false,
				})
...
const generateSystemPrompt = async (provider: ClineProvider, message: WebviewMessage) => {
	...
	const lunaIgnoreInstructions = provider.getCurrentCline()?.lunaIgnoreController?.getInstructions()
