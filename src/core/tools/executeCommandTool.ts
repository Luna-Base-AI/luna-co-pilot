...
export async function executeCommandTool(
	...
	try {
		if (block.partial) {
			...
			const ignoredFileAttemptedToAccess = cline.lunaIgnoreController?.validateCommand(command)

			if (ignoredFileAttemptedToAccess) {
				await cline.say("lunaignore_error", ignoredFileAttemptedToAccess)
				pushToolResult(formatResponse.toolError(formatResponse.lunaIgnoreError(ignoredFileAttemptedToAccess)))
				return
			}
