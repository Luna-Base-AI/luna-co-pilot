...
import { LunaIgnoreController, LOCK_TEXT_SYMBOL } from "../ignore/LunaIgnoreController"
...
export const formatResponse = {
	...
	lunaIgnoreError: (path: string) =>
		`Access to ${path} is blocked by the .lunaignore file settings. You must try to continue in the task without using this file, or ask the user to update the .lunaignore file.`,
...
(This is an automated message, so do not respond to it conversationally.)`,
	...
	formatFilesList: (
		...
		files: string[],
		didHitLimit: boolean,
		lunaIgnoreController: LunaIgnoreController | undefined,
		showLunaIgnoredFiles: boolean,
	): string => {
		const sorted = files
			.map((file) => {
		...
		let lunaIgnoreParsed: string[] = sorted

		if (lunaIgnoreController) {
			lunaIgnoreParsed = []
			for (const filePath of sorted) {
				...
				const isIgnored = !lunaIgnoreController.validateAccess(absoluteFilePath)

				if (isIgnored) {
					// If file is ignored and we're not showing ignored files, skip it
					if (!showLunaIgnoredFiles) {
						continue
					}
					// Otherwise, mark it with a lock symbol
					lunaIgnoreParsed.push(LOCK_TEXT_SYMBOL + " " + filePath)
				} else {
					lunaIgnoreParsed.push(filePath)
				}
			}
		}
		if (didHitLimit) {
			return `${lunaIgnoreParsed.join(
				"\n",
			)}\n\n(File list truncated. Use list_files on specific subdirectories if you need to explore further.)`
		} else if (lunaIgnoreParsed.length === 0 || (lunaIgnoreParsed.length === 1 && lunaIgnoreParsed[0] === "")) {
