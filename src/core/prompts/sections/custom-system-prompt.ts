...
export function getSystemPromptFilePath(cwd: string, mode: Mode): string {
	return path.join(cwd, ".luna", `system-prompt-${mode}`)
}

/**
 * Loads custom system prompt from a file at .luna/system-prompt-[mode slug]
...
/**
 * Ensures the .luna directory exists, creating it if necessary
 */
export async function ensureLunaDirectory(cwd: string): Promise<void> {
	const lunaDir = path.join(cwd, ".luna")

	// Check if directory already exists
	if (await fileExistsAtPath(lunaDir)) {
		return
	}
