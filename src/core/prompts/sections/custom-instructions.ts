...
export async function loadRuleFiles(cwd: string): Promise<string> {
	// Check for .luna/rules/ directory
	const lunaRulesDir = path.join(cwd, ".luna", "rules")
	if (await directoryExists(lunaRulesDir)) {
		const files = await readTextFilesFromDirectory(lunaRulesDir)
		if (files.length > 0) {
			return formatDirectoryContent(lunaRulesDir, files)
		}
	}

	// Fall back to existing behavior
	const ruleFiles = [".lunarules", ".clinerules"]
...
export async function addCustomInstructions(
	...
	cwd: string,
	mode: string,
	options: { language?: string; lunaIgnoreInstructions?: string } = {},
): Promise<string> {
	const sections = []
	...
	if (mode) {
		// Check for .luna/rules-${mode}/ directory
		const modeRulesDir = path.join(cwd, ".luna", `rules-${mode}`)
		...
		if (!modeRuleContent) {
			const lunaModeRuleFile = `.lunarules-${mode}`
			modeRuleContent = await safeReadFile(path.join(cwd, lunaModeRuleFile))
			if (modeRuleContent) {
				usedRuleFile = lunaModeRuleFile
			} else {
	...
	if (modeRuleContent && modeRuleContent.trim()) {
		if (usedRuleFile.includes(path.join(".luna", `rules-${mode}`))) {
	...
	if (options.lunaIgnoreInstructions) {
		rules.push(options.lunaIgnoreInstructions)
	}
