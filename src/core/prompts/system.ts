...
${await addCustomInstructions(promptComponent?.customInstructions || modeConfig.customInstructions || "", globalCustomInstructions || "", cwd, mode, { language: language ?? formatLanguage(vscode.env.language), lunaIgnoreInstructions })}`
...
export const SYSTEM_PROMPT = async (
	...
	if (fileCustomSystemPrompt) {
		...
		const customInstructions = await addCustomInstructions(
			...
			cwd,
			mode,
			{ language: language ?? formatLanguage(vscode.env.language), lunaIgnoreInstructions },
		)
