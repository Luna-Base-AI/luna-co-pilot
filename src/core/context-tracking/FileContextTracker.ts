...
// If a user modifies a file outside of Luna Co-Pilot, the context may become stale and need to be updated.
// We do not want Luna Co-Pilot to reload the context every time a file is modified, so we use this class merely
// to inform Luna Co-Pilot that the change has occurred, and tell Luna Co-Pilot to reload the file before making
// any changes to it. This fixes an issue with diff editing, where Luna Co-Pilot was unable to complete a diff edit.

// FileContextTracker
//
// This class is responsible for tracking file operations.
// If the full contents of a file are passed to Luna Co-Pilot via a tool, mention, or edit, the file is marked as active.
// If a file is modified outside of Luna Co-Pilot, we detect and track this change to prevent stale context.
export class FileContextTracker {
	...
	private recentlyEditedByLuna = new Set<string>()
	...
	async setupFileWatcher(filePath: string) {
		...
		// Track file changes
		watcher.onDidChange(() => {
			if (this.recentlyEditedByLuna.has(filePath)) {
				this.recentlyEditedByLuna.delete(filePath) // This was an edit by Luna, no need to inform Luna
			} else {
				this.recentlyModifiedFiles.add(filePath) // This was a user edit, we will inform Luna
	...
	// This is the main entry point for FileContextTracker and is called when a file is passed to Luna via a tool, mention, or edit.
	...
	async addFileToFileContextTracker(taskId: string, filePath: string, source: RecordSource) {
		try {
			...
			let newEntry: FileMetadataEntry = {
				...
				luna_read_date: getLatestDateForField(filePath, "luna_read_date"),
				luna_edit_date: getLatestDateForField(filePath, "luna_edit_date"),
			...
			switch (source) {
				...
				// luna_edited: luna has edited the file
				case "luna_edited":
					newEntry.luna_read_date = now
					newEntry.luna_edit_date = now
					this.checkpointPossibleFiles.add(filePath)
					break

				// read_tool/file_mentioned: luna has read the file via a tool or file mention
				case "read_tool":
				case "file_mentioned":
					newEntry.luna_read_date = now
					break
	...
	// Marks a file as edited by Luna to prevent false positives in file watchers
	markFileAsEditedByLuna(filePath: string): void {
		this.recentlyEditedByLuna.add(filePath)
	}
