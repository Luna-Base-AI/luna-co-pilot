...
import { LunaIgnoreController } from "./ignore/LunaIgnoreController"
...
export class Cline extends EventEmitter<ClineEvents> {
	...
	lunaIgnoreController?: LunaIgnoreController
	...
	constructor({
		...
		this.lunaIgnoreController = new LunaIgnoreController(this.cwd)
		this.fileContextTracker = new FileContextTracker(provider, this.taskId)
		this.lunaIgnoreController.initialize().catch((error) => {
			console.error("Failed to initialize LunaIgnoreController:", error)
		})
	...
	async abortTask(isAbandoned = false) {
		...
		this.lunaIgnoreController?.dispose()
	...
	async *attemptApiRequest(previousApiReqIndex: number, retryAttempt: number = 0): ApiStream {
		...
		const lunaIgnoreInstructions = this.lunaIgnoreController?.getInstructions()
		...
		if (this.consecutiveMistakeCount >= this.consecutiveMistakeLimit) {
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "Luna Co-Pilot uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
	...
	async getEnvironmentDetails(includeFileDetails: boolean = false) {
		...
		// Filter paths through lunaIgnoreController
		const allowedVisibleFiles = this.lunaIgnoreController
			? this.lunaIgnoreController.filterPaths(visibleFilePaths)
		...
		// Filter paths through lunaIgnoreController
		const allowedOpenTabs = this.lunaIgnoreController
			? this.lunaIgnoreController.filterPaths(openTabPaths)
		...
		*/
		...
		if (includeFileDetails) {
			...
			if (isDesktop) {
				...
				const { showLunaIgnoredFiles = true } = (await this.providerRef.deref()?.getState()) ?? {}
				const result = formatResponse.formatFilesList(
					this.cwd,
					files,
					didHitLimit,
					this.lunaIgnoreController,
