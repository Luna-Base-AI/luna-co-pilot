...
import { LunaCoPilotSettings, LunaCoPilotEvents, LunaCoPilotEventName, ClineMessage } from "../schemas"
import { IpcOrigin, IpcMessageType, TaskCommandName, TaskEvent } from "../schemas/ipc"

import { LunaCoPilotAPI } from "./interface"
import { IpcServer } from "./ipc"
import { outputChannelLog } from "./log"

export class API extends EventEmitter<LunaCoPilotEvents> implements LunaCoPilotAPI {
	...
	constructor(
		...
		if (enableLogging) {
			...
			this.logfile = path.join(getWorkspacePath(), "luna-co-pilot-messages.log")
		} else {
			this.log = () => {}
		}
	...
	public override emit<K extends keyof LunaCoPilotEvents>(
		eventName: K,
		...args: K extends keyof LunaCoPilotEvents ? LunaCoPilotEvents[K] : never
	) {
		const data = { eventName: eventName as LunaCoPilotEventName, payload: args } as TaskEvent
	...
	public async startNewTask({
		...
		configuration: LunaCoPilotSettings
		text?: string
		images?: string[]
		newTab?: boolean
	}) {
		...
		if (newTab) {
			...
			await vscode.commands.executeCommand("luna-co-pilot.SidebarProvider.focus")
		...
		if (configuration) {
			...
			if (configuration.allowedCommands) {
				await vscode.workspace
					.getConfiguration("luna-co-pilot")
	...
	public async setConfiguration(values: LunaCoPilotSettings) {
	...
	public async deleteProfile(name: string) {
		...
		const newSettings: LunaCoPilotSettings = {
			...currentSettings,
	...
	private registerListeners(provider: ClineProvider) {
		provider.on("clineCreated", (cline) => {
			cline.on("taskStarted", async () => {
				this.emit(LunaCoPilotEventName.TaskStarted, cline.taskId)
			...
			cline.on("message", async (message) => {
				this.emit(LunaCoPilotEventName.Message, { taskId: cline.taskId, ...message })
			...
			cline.on("taskModeSwitched", (taskId, mode) =>
				this.emit(LunaCoPilotEventName.TaskModeSwitched, taskId, mode),
			)

			cline.on("taskAskResponded", () => this.emit(LunaCoPilotEventName.TaskAskResponded, cline.taskId))

			cline.on("taskAborted", () => {
				this.emit(LunaCoPilotEventName.TaskAborted, cline.taskId)
				this.taskMap.delete(cline.taskId)
			})

			cline.on("taskCompleted", async (_, tokenUsage, toolUsage) => {
				this.emit(LunaCoPilotEventName.TaskCompleted, cline.taskId, tokenUsage, toolUsage)
				this.taskMap.delete(cline.taskId)
			...
			cline.on("taskSpawned", (childTaskId) =>
				this.emit(LunaCoPilotEventName.TaskSpawned, cline.taskId, childTaskId),
			)
			cline.on("taskPaused", () => this.emit(LunaCoPilotEventName.TaskPaused, cline.taskId))
			cline.on("taskUnpaused", () => this.emit(LunaCoPilotEventName.TaskUnpaused, cline.taskId))

			cline.on("taskTokenUsageUpdated", (_, usage) =>
				this.emit(LunaCoPilotEventName.TaskTokenUsageUpdated, cline.taskId, usage),
			)

			cline.on("taskToolFailed", (taskId, tool, error) =>
				this.emit(LunaCoPilotEventName.TaskToolFailed, taskId, tool, error),
			)

			this.emit(LunaCoPilotEventName.TaskCreated, cline.taskId)
		})
	}
